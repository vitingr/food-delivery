...
const page = () => {
  ...
  const getRestaurants = async () => {
    const requisition = await fetch(
      "https://food-delivery-nest-api.vercel.app/restaurant"
    );
    ...
    products.length > 0 && (
      <div className="overflow-hidden w-full flex flex-col items-center sm:p-[2%]">
        <section className="w-full flex flex-col items-center justify-center p-[2%]">
          <div className="w-full max-w-[1600px]">
            ...
            <div className="flex sm:flex-nowrap flex-wrap gap-6">
              {FEATURED_RESTAURANTS.map(
                (restaurant: RestaurantProps | any, index: number) => (
                  <FeaturedRestaurant
                    image={restaurant.image}
                    name={restaurant.name}
                    branch={restaurant.branch}
                    verified={restaurant.verified}
                    linkPage={restaurant.linkPage}
                    key={index}
                  />
                )
              )}
        ...
        <section className="w-[2600px]  overflow-hidden mt-[50px] flex flex-col items-center">
          <h2 className="w-full text-3xl selection:bg-[#ea1d2c] selection:text-white max-w-[1600px] mb-6">
            Destaques
          </h2>
        ...
        <section className="max-w-[1600px] w-full mt-[100px]">
          <h2 className="sm:text-2xl text-4xl selection:bg-[#ea1d2c] selection:text-white text-center sm:text-left">
            Você pode gostar
          </h2>
          <h5 className="text-[#717171] sm:text-sm text-base selection:bg-[#ea1d2c] selection:text-white text-center sm:text-left">
            Escolha o que você mais deseja
          </h5>
          <div className="w-full flex flex-wrap sm:justify-normal justify-center sm:gap-6 gap-14 mt-14">
            {RESTAURANT_OPTIONS.map((restaurant: any, index: number) => (
              <SmallRestaurantOption
        ...
        <section className="mt-[100px] max-w-[1600px] w-full">
          <h2 className="sm:text-2xl text-4xl selection:bg-[#ea1d2c] selection:text-white text-center sm:text-left">
            Mais Food Delivery para você
          </h2>
          <h5 className="text-[#717171] sm:text-sm text-base selection:bg-[#ea1d2c] selection:text-white text-center sm:text-left">
            Veja mais algumas opções especiais
          </h5>
          <div className="w-full flex flex-wrap sm:gap-6 gap-14 mt-14">
            ...
            >
              <img
                src="https://static.ifood-static.com.br/image/upload/t_medium/discoveries/03cupons_ESAv.png?imwidth=128"
                className="w-[100px]"
                alt="Cupom Image"
              />
        ...
        <section className="mt-[100px] max-w-[1600px] w-full overflow-hidden">
          ...
          <h5 className="text-[#717171] sm:text-sm text-base selection:bg-[#ea1d2c] selection:text-white text-center sm:text-left">
            Veja algumas opções de estabelecimentos
          </h5>
        ...
        <section className="mt-[200px] mb-[150px] w-full flex justify-center">
          <img
            ...
            alt="Logo"
            className="w-full max-w-[1600px] cursor-pointer"
          />
        </section>
      </div>
    )
