...
const page = () => {
  ...
  useEffect(() => {
    if (!isFetched.current) {
      if (query && status === "authenticated" && session?.user?.email !== undefined) {
        getRestaurants()
        isRestaurantOpen()
      }
    } else {
    ...
    <div className='p-[5%] w-full flex flex-col items-center'>
      ...
      {restaurants.length > 0 ? (
        ...
        <div className='max-w-[1500px] flex flex-col items-center'>
          <h1 className='font-semibold selection:bg-[#ea1d2c] selection:text-white text-center'>Nenhum restaurante encontrado</h1>
          <h3 className='mt-4 text-[#717171] selection:bg-[#ea1d2c] selection:text-white text-center'>Que tal ver outras opções?</h3>
