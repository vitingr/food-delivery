...
const page = () => {
    ...
    if (data.id !== undefined) {
      try {
        ...
        const response = await fetch(
          "https://food-delivery-nest-api.vercel.app/user/update",
          {
            method: "POST",
            headers: {
    ...
    if (data.id !== null && data.id !== undefined) {
      try {
        const result = await fetch(
          `https://food-delivery-nest-api.vercel.app/address/${data.id}`
        );
  ...
  const createAddress = async (
    ...
    if (
      ...
      try {
        const response = await fetch(
          "https://food-delivery-nest-api.vercel.app/address/create",
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
    ...
    <div className="p-[5%] w-full flex flex-col items-center">
      ...
      >
        <div className="flex flex-col mt-10 mb-10 w-full">
          <label
            htmlFor="logo"
            className="text-lg selection:bg-[#ea1d2c] selection:text-white"
          >
            Sua Foto
          </label>
          <Upload
            currentFoto={data.photo}
            setState={setPhoto}
            styles="w-full absolute opacity-0 cursor-pointer sm:max-w-[500px] max-w-[240px] mt-[10px] "
          />
        </div>
        ...
        <div>
          ...
          <div
            className="text-[#ea1d2c] underline underline-offset-4 text-center w-full cursor-pointer mb-4"
        ...
        {creatingAddress ? (
          <Popup state={setCreatingAddress} title={`Adicionar Endereço`}>
            ...
            >
              ...
              <button
                className="mt-6 w-full bg-[#ea1d2c] rounded-xl p-4 text-center text-white font-bold cursor-pointer"
                type="submit"
              >
                Adicionar Endereço
              </button>
        ...
        addressId != "" ? (
          <button
            className="mt-28 w-full border text-[#ea1d2c] border-[#ea1d2c] rounded-xl p-4 text-center font-bold cursor-pointer"
            type="submit"
          >
            Editar Informações
          </button>
          ...
          <button
            className="mt-28 w-full bg-[#dddddd] rounded-xl p-4 text-center text-[#717171] font-bold cursor-not-allowed"
            type="submit"
          >
            Editar Informações
          </button>
