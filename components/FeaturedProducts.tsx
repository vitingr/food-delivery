...
const FeaturedProducts = ({ productData }: FeaturedProduct) => {
  return (
    <Link href={`/restaurant/${productData.restaurant}`}>
      <div className='p-6 border border-neutral-200 rounded-lg w-[275px] h-[440px] shadow-sm'>
        <img src={productData.productFoto} className="w-full h-[170px] rounded-md" alt="Product Photo" />
        <h1 className='selection:bg-[#ea1d2c] selection:text-white text-lg mt-4'>{productData.productName}</h1>
        <p className='text-xs text-[#717171] mt-4 h-[60px] overflow-hidden'>{productData.productDescription}</p>
