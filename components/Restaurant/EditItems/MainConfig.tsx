...
const MainConfig = ({
  ...
  const [speciality, setSpeciality] = useState<string>(
    restaurantData.speciality
  );
  const [delivery, setDelivery] = useState<string>(restaurantData.delivery);
  const [deliveryValue, setDeliveryValue] = useState<number>(
    restaurantData.deliveryValue
  );
  ...
  const [closingHour, setClosingingHour] = useState<number>(
    restaurantData.closingHour
  );

  // Input Masks
  const [cpfFormated, setCpfFormated] = useState<string>("");
  let deliveryValueFormated = deliveryValue.toLocaleString("pt-BR", {
    style: "currency",
    currency: "BRL",
  });
  ...
  const buscaCidadesPorEstado = (estado: string) => {
    ...
    ); // Vai procurar no json estados, um estado com o sigla igual ao estado Selecionado
      ...
      if (imageUpload.ok && backgroundUpload.ok) {
        ...
        try {
          const response = await fetch(
            "https://food-delivery-nest-api.vercel.app/restaurant/update",
            {
              method: "POST",
              headers: {
  ...
  return (
    <div className="flex w-full justify-center">
      <ToastMessage />
      <form
        className="max-w-[750px] w-full mt-16"
        onSubmit={(e: React.SyntheticEvent) => {
          e.preventDefault();
          updateInfo();
        }}
      >
        <label htmlFor="deliveryValue" className="text-lg">
          Valor de entrega do Restaurante
        </label>
        <input
          type="number"
          name="deliveryValue"
          ...
          autoComplete="off"
          placeholder="Qual é o valor de entrega do restaurante?"
        ...
        <label htmlFor="minValue" className="text-lg">
        ...
        <input
          ...
          autoComplete="off"
          placeholder="Qual é o tempo médio de entrega do restaurante"
          onChange={(e) => setMinValue(Number(e.target.value))}
          required
        />

        <label htmlFor="deliveryTime" className="text-lg">
        ...
        <div className="flex flex-col mt-10 mb-10 max-w-[500px]">
          ...
          <Upload
            currentFoto={logo}
            setState={setLogo}
            styles="w-full opacity-0 cursor-pointer max-w-[375px] mt-[10px]"
            text="Enviar logo"
          />

          <label htmlFor="background" className="text-lg mt-10">
            Seu Plano de Fundo
          </label>
          <Upload
            currentFoto={background}
            setState={setBackground}
            styles="w-full opacity-0 cursor-pointer max-w-[375px] mt-[10px]"
            text="Enviar background"
          />
        </div>

        <label htmlFor="telefone" className="text-lg">
          Celular do Dono (com DDD)
        </label>
        <input
          type="text"
          name="telefone"
          ...
          autoComplete="off"
          placeholder="Qual é o celular do restaurante"
        ...
        <div className="flex items-center gap-6">
          <div className="w-full">
            <label htmlFor="abertura" className="text-lg">
              Hora de Abertura do Restaurante
            </label>
            <input
              type="number"
              name="abertura"
              ...
              className="w-full outline-none pl-4 pr-4 pt-2 pb-2 border border-neutral-200 rounded-lg mt-1 text-[#717171] mb-8"
              autoComplete="off"
              placeholder="Hora da abertura do restaurante"
              onChange={(e) => changeOpeningHour(e.target.value)}
              required
            />
          </div>
          <div className="w-full">
            <label htmlFor="fechamento" className="text-lg">
              Hora de Fechamento do Restaurante
            </label>
            <input
              type="number"
              name="fechamento"
              ...
              className="w-full outline-none pl-4 pr-4 pt-2 pb-2 border border-neutral-200 rounded-lg mt-1 text-[#717171] mb-8"
              autoComplete="off"
              placeholder="Hora do fechamento do restaurante"
              onChange={(e) => changeClosingHour(e.target.value)}
              required
            />
          </div>
        </div>
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="w-full">
            <label htmlFor="sobrenome" className="text-lg">
              Sobrenome
            </label>
            <input
              type="text"
        ...
        <label htmlFor="restauranteName" className="text-lg">
          Nome do restaurante
        </label>
        <input
          type="text"
        ...
        <label htmlFor="telephone" className="text-lg">
          Celular do Restaurante (com DDD)
        </label>
        <input
          type="text"
          name="telephone"
          ...
          onChange={(e) => verifyCellphoneRestaurant(e.target.value)}
          required
        />
        ...
        <div className="flex items-center gap-6">
          <div className="w-full">
            ...
            <input
              ...
              defaultValue={street}
              autoComplete="off"
              placeholder="Exemplo: Bairro ou Rua do restaurante"
        ...
        <label htmlFor="speciality" className="text-lg">
          Especialidade
        </label>
        <select
          name="speciality"
        ...
        >
          <option value="">
            Selecione a especialidade principal do restaurante
          </option>
        ...
        <label htmlFor="delivery" className="text-lg">
          O restaurante possui serviço de entrega?
        </label>
              ...
              <>
                <div className="flex gap-4 items-center">
                  <input
                    ...
                    id="delivery"
                    value={"Sim"}
                    className="accent-[#ea1d2c] text-white w-[15px] h-[15px] cursor-pointer"
        ...
        <button
          type="submit"
          className="mt-12 w-full bg-[#ea1d2c] rounded-xl p-4 text-center text-white font-bold cursor-pointer"
        >
